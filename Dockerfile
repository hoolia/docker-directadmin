...
RUN echo 1 > /root/.lan && \
    curl -sSL http://www.directadmin.com/setup.sh | sh -s $UID $LID $HOSTNAME eth0 

WORKDIR /usr/local/directadmin
RUN curl -sSL http://files.directadmin.com/services/custombuild/1.1/custombuild.tar.gz |tar xz && \
    cd custombuild && \
    chmod +x build && \
    ./build update_data && \
    ./build all n
...
WORKDIR /usr/local/directadmin/data
# . /usr/local/directadmin/conf/mysql.conf #loading admin username/password
RUN echo "oversell=yes\nsuspend=no" >>admin/admin.conf && \
...
RUN /usr/local/directadmin/scripts/majordomo.sh

RUN /usr/local/directadmin/scripts/spam.sh
...
CMD ["/etc/init.d/directadmin","start"]
